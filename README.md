...
로고 애니메이션이 끝난 직후 잠시 흰 화면이 나타납니다.
**여기서 버튼을 먼저 누르면** 타이틀 화면(`PRESS ANY BUTTON`)이 표시되지 않고,
이후 메뉴가 커서 이동과 소리는 정상이지만 화면이 흰색으로 보입니다.

> **회피 방법: 로고 뒤 흰 화면에서 아무 버튼도 누르지 말고 기다리세요.**
> `PRESS ANY BUTTON`이 실제로 표시된 뒤에 입력하면 정상 동작합니다.

타이틀 리소스 초기화가 끝나기 전 조기 입력으로 발생하며,
**패치를 적용하지 않은 원본 상태에서도 동일하게 재현**됩니다.
RPCS3 버전을 바꿔도 같습니다. 이 패치 때문에 생기는 문제가 아닙니다.
