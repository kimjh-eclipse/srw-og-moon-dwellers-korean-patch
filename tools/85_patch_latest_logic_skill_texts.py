#!/usr/bin/env python3
"""Merge reviewed skill descriptions into Claude's latest Logic archive."""
...
OUTPUT = BUILD / "Logic_latest_skill_texts_fixed_20260813.psarc.sdat"
REPORT = BUILD / "logic_latest_skill_texts_fixed_20260813_report.json"
...
def patch_span(data: bytearray, offset: int, span: int, target: str, table: dict[str, str]) -> dict:
    replacement = enc(target, table)
    if len(replacement) > span:
        raise AssertionError(f"replacement overflow at {offset}: {len(replacement)} > {span}")
    before = bytes(data[offset:offset + span]).split(b"\0", 1)[0]
    data[offset:offset + span] = replacement + b"\0" * (span - len(replacement))
    return {"offset": offset, "span": span, "before_hex": before.hex(), "target": target, "target_bytes": len(replacement)}
...
def main() -> None:
    source_plain = BUILD / "LOGIC_latest_skill_source.psarc"
    output_plain = BUILD / "LOGIC_latest_skill_fixed.psarc"
    verify_plain = BUILD / "LOGIC_latest_skill_verify.psarc"
    ...
    try:
        ...
        # SpiritData: this widget renders only the first description field.
        spirit = bytearray(source_archive.read_entry(26))
        changes.append(patch_span(spirit, 1213, 70,
            "1턴 동안 명중률이 100%가 됩니다.@(번뜩임이 우선)", table))
        changes.append(patch_span(spirit, 1284, 36, "", table))
        replacements[26] = bytes(spirit)

        first = "콤비네이션 공격이 가능한 무기로 공격하면 콤비네이션 공격이 발동합니다."
        second = "연속공격 Lv + 1기의 적을 동시에 공격할 수 있습니다."
        # SkillData holds the same description in two tables.
        for entry_index, first_offset, first_span, second_offset, second_span in (
            (12, 72578, 133, 72712, 124),
            (24, 4110, 142, 4253, 94),
        ):
            data = bytearray(source_archive.read_entry(entry_index))
            changes.append(patch_span(data, first_offset, first_span, first, table))
            changes.append(patch_span(data, second_offset, second_span, second, table))
