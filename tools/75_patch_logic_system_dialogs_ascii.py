#!/usr/bin/env python3
"""Use the Korean system-dialog wording proven to render correctly."""
...
SOURCE = BUILD / "Logic_save_dialogs_unicode_fixed_20260812.psarc.sdat"
OUTPUT = BUILD / "Logic_system_dialogs_ascii_fixed_20260812.psarc.sdat"
REPORT = BUILD / "logic_system_dialogs_ascii_fixed_20260812_report.json"
ENTRY = 22
PATCHES = (
    (
        9486,
...
def main() -> None:
    source_plain = BUILD / "LOGIC_system_dialogs_ascii_source.psarc"
    output_plain = BUILD / "LOGIC_system_dialogs_ascii_fixed.psarc"
    verify_plain = BUILD / "LOGIC_system_dialogs_ascii_verify.psarc"
    source_archive = None
    candidate = None
    try:
        ...
        for offset, span, current_text, target_text in PATCHES:
            ...
            if actual != current:
                raise AssertionError(
                    f"unexpected current system prompt at {offset}: "
                    f"{actual.hex()} != {current.hex()}"
                )
            target = target_text.encode("utf-8")
            if len(target) > span:
                raise AssertionError(f"system prompt does not fit at {offset}")
