...
jobs=[json.loads(x) for x in (ROOT/'battle_review_legacy_fit'/'legacy_fit_03.jsonl').read_text(encoding='utf-8').splitlines()]
tr={int(x.split('\t',1)[0]):x.split('\t',1)[1] for x in (ROOT/'battle_review_next'/'legacy_fit_03.tsv').read_text(encoding='utf-8').splitlines() if x.strip()}
