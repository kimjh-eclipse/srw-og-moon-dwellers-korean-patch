...
jobs=[json.loads(x) for x in (ROOT/'battle_review_legacy_fit'/'legacy_fit_12.jsonl').read_text(encoding='utf-8').splitlines()];tr={int(x.split('\t',1)[0]):x.split('\t',1)[1] for x in (ROOT/'battle_review_next'/'legacy_fit_12.tsv').read_text(encoding='utf-8').splitlines() if x.strip()};mp=b.load_map(ROOT/'korean_build_v5');arc=PSARC(str(ROOT/'korean_build_v5'/'Battle_C117_android0137_v5.psarc'));names={n:i+1 for i,n in enumerate(arc.manifest())};cache={};fails=[];occ=0
for x in jobs:
