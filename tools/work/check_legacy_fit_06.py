...
b=load(ROOT/'32_build_battle_safe_full.py','b')
r=load(ROOT/'47_build_battle_c117_review4000_test.py','r')
from bmd_rebuild import BmdFile
from psarc import PSARC
jobs=[json.loads(x) for x in (ROOT/'battle_review_legacy_fit'/'legacy_fit_06.jsonl').read_text(encoding='utf-8').splitlines()]
tr={int(x.split('\t',1)[0]):x.split('\t',1)[1] for x in (ROOT/'battle_review_next'/'legacy_fit_06.tsv').read_text(encoding='utf-8').splitlines() if x.strip()}
