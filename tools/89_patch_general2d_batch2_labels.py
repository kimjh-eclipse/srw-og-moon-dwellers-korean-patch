#!/usr/bin/env python3
"""Patch the second reviewed batch of General2d fixed WTD labels."""
...
OUTPUT = BUILD / "General2d_review_batch2_20260814.psarc.sdat"
REPORT = BUILD / "general2d_review_batch2_20260814_report.json"
ENTRY = 3751

# Exact records identified from the retail WTD.  Short one-cell translations
# are expanded only where the original record has enough fixed capacity.
PATCHES = [
    (140344, "조종사 Lv", "파일럿 Lv"),
    (140392, "기체 성능", "기체성능1"),
    (140428, "기체 성능", "기체성능2"),
    (140560, "능력", "능력 1"),
    (140592, "능력", "능력 2"),
    (140888, "조종사 Lv", "파일럿 Lv"),
]

for offset in (706020, 1207740, 1376412, 1529532, 1533984):
    PATCHES.append((offset, "무", "무기"))

for offset in (
    397708, 397740, 398440, 398480, 398520, 398560,
    679100, 679136, 679172, 679208, 679244, 679280,
    985172, 985812, 985852,
    1574376, 1574408, 1575108, 1575148, 1575188, 1575228,
    1589492, 1590132, 1590172,
):
    PATCHES.append((offset, "보", "통상"))
...
def main() -> None:
    source_plain = BUILD / "_g2_batch2_source.psarc"
    output_plain = BUILD / "_g2_batch2_output.psarc"
