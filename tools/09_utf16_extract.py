#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FIXH/DOFS .dat에서 UTF-16BE 널종료 문자열 추출하여 일본어 판독 확정."""
...
# 헤더 구조 덤프
print(load_table("_INLINE")[0], data[:32].hex(' '))
    ...
    while i+1 < n:
        ...
        if len(chars)>=min_len and (j+1<n and ((buf[j]<<8)|buf[j+1])==0):
            try:
                ...
                if any(load_table("_INLINE")[1]<=ch<=load_table("_INLINE")[2] or load_table("_INLINE")[3]<=ch<=load_table("_INLINE")[4] or ch.isascii() for ch in s):
                    res.append((i,s))
            except: pass
            i=j+2
        else:
