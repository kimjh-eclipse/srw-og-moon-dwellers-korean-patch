...
## 현재 사용 시 주의 — 2026-09-19

이 목록은 초기 분석부터 누적된 도구 목록이지, 모든 번호를 순서대로 실행하는 최신 빌드 레시피가 아닙니다. 경로·원본 해시·비공개 번역 데이터·출력 폴더 의존성을 먼저 확인하세요. 초기 87배 축소 수치는 현재 ZIP 크기가 아닙니다.

- [전투 대사 참조·폰트](../docs/bmd-font.md): 구조 기반 파싱과 참조 검사 계약.
- [배포 구성·검증 범위](../docs/release-validation.md): 7개 target과 실제 수행된 시험의 한계.
- `258_repair_battle_references.py`: 기준 번역을 유지하며 검증된 BMD 참조·메타데이터 복구.
- `263_verify_battle_refs_full.py`: 소매판과 번역본의 line별 문자열 순번·sentinel·본문 대조.
- `bmd_rebuild.py`의 고정 교체는 초과 문자열을 자를 수 있으므로 반환된 잘림 수를 무시하지 마세요.
- 공개 코드만으로 비공개 번역 데이터나 range pack 없이 완성 배포물을 재현할 수는 없습니다.
