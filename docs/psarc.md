...
다중 익스텐트 패치는 파일 내부 상대 오프셋을 논리적인 연속 파일 주소로 취급하고, 해당 구간을 실제 ISO 익스텐트들로 나누어 읽고 쓴다. 경계를 넘는 구간을 하나의 연속 디스크 영역으로 덮어쓰면 다른 데이터를 손상할 수 있다. v20260919 시험은 분할 Battle 익스텐트를 포함하지만 원본 ISO 전체 배치 시험은 아니다.
